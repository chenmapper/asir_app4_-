...
st.title("📘 asir_app4_通用相片索引 v0.26（雲端專用修正版）")
...
generate_btn = st.button("🧾 產生索引表與圖片壓縮包")
...
if generate_btn and uploaded_zip:
    zip_name = Path(uploaded_zip.name).stem  # zip 檔名（不含副檔名）
    ...
    for img_path in image_paths:
        try:
            ...
            full_file_path = str(Path(custom_root) / zip_name / img_path.name)
    ...
    st.download_button("⬇️ 下載相片索引表 (.xlsx)", data=xlsx_buffer.getvalue(),
                       file_name=f"{zip_name}_相片索引表.xlsx",
                       mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")

    st.download_button("⬇️ 下載圖檔壓縮包 (.zip)", data=zip_buffer.getvalue(),
