...
# --- Configuration ---
st.set_page_config(page_title="📘 asir_app4_通用相片索引 v0.38 子任務2：原欄位更新（加入根目錄設定）", layout="wide")
...
# Processing function
def process_batch(zip_file, xlsx_file, root_dir):
    with tempfile.TemporaryDirectory() as extract_dir, tempfile.TemporaryDirectory() as output_dir:
        ...
        # 補齊副檔名
        def ensure_ext(new, old):
            if not new:
                return old
            return new if Path(new).suffix else f"{new}{Path(old).suffix}"
        new_names = [ensure_ext(n, o) for n, o in zip(df['新檔名'], old_names)]

        # 更新欄位內容
        df['原檔名'] = new_names
        df['新檔名'] = [f"({new}/{old})" for new, old in zip(new_names, old_names)]
        # 更新原圖路徑：加上 file:/// 前綴 + 根目錄 + zip 內目錄 + 新檔名
        df['原圖路徑'] = [
            f"file:///{Path(root_dir) / upload_dir / new}" if upload_dir else f"file:///{Path(root_dir) / new}"
        ...
        for old, new in zip(old_names, new_names):
            src = base_path / old
            dst = Path(output_dir) / new
            if src.exists():
                shutil.copy(src, dst)
                logs.append(f"✅ {old} → {new}")
            else:
                logs.append(f"⚠️ 檔案不存在: {old}")
        ...
        with zipfile.ZipFile(zip_buffer, 'w', zipfile.ZIP_DEFLATED) as out_z:
            for file in Path(output_dir).iterdir():
                out_z.write(file, arcname=file.name)
        zip_buffer.seek(0)
...
if uploaded_zip and uploaded_xlsx:
    if st.button('✅ 執行更新'):
        zip_data, excel_data, logs = process_batch(uploaded_zip, uploaded_xlsx, root_dir)
...
if st.session_state['zip_data']:
    st.download_button(
        ...
        file_name=st.session_state['orig_zip_name'],
...
if st.session_state['excel_data']:
    st.download_button(
        ...
        file_name=st.session_state['orig_xlsx_name'],
