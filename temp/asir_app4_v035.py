...
st.set_page_config(page_title="📘 asir_app4_通用相片索引 v0.35", layout="wide")
st.title("📘 asir_app4_通用相片索引 v0.35（雲端補強版）")
...
if uploaded_zip and uploaded_xlsx:
    ...
    if run_btn:
        ...
        st.session_state["excel_buffer"] = output_excel.getvalue()

        st.success(f"📦 res{i} 資料夾建立完成，包含圖檔與索引表")

# 無論是否重新執行，保留按鈕
if st.session_state["zip_buffer"]:
    st.download_button("⬇️ 下載整包圖庫與索引表", data=st.session_state["zip_buffer"], file_name="resX.zip", mime="application/zip")

if st.session_state["excel_buffer"]:
    st.download_button("⬇️ 下載相片索引表.xlsx", data=st.session_state["excel_buffer"], file_name="相片索引表.xlsx", mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")
