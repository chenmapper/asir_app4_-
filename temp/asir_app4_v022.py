...
st.title("📘 asir_app4_通用相片索引 v0.22（雲端部署模擬）")

uploaded_zip = st.file_uploader("📦 上傳圖片資料夾（zip格式）", type="zip")
st.markdown("📁 預設圖片儲存路徑：")
default_path = "C:/Users/User/Downloads/table_app/case4/temp_images/res"
custom_path = st.text_input("🔧 自訂圖片根目錄（將寫入 Excel 原圖路徑欄，並產生超連結）", value=default_path)
...
if generate_btn and uploaded_zip:
    ...
    for img_path in image_paths:
        try:
            ...
            full_file_path = f"{custom_path}/更新後圖片/{img_path.name}"
            file_url = "file:///" + full_file_path.replace("\", "/")  # 超連結格式
