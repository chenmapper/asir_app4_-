...
st.set_page_config(page_title="📘 相片索引表 v1.6", layout="wide")
st.title("📘 相片索引表 v1.6")
    ...
    if uploaded_zip and uploaded_excel:
        try:
            ...
            if "原檔名" not in df.columns or "新/舊檔名" not in df.columns:
                ...
                res_dir, res_img_dir = get_next_res_dir()
                valid_rows = []
                ...
                # 顯示提示與下載按鈕
                st.success(f"✅ 已完成，資料儲存於：{res_dir}")
                with open(xlsx_path, "rb") as f:
                    st.download_button("📄 下載相片索引表", data=f.read(),
