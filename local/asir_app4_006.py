...
st.markdown("上傳你的相片資料夾（ZIP），建立圖片索引、說明、雜湊指紋")
...
if uploaded_zip:
    ...
    for i, img_path in enumerate(image_paths):
        try:
            ...
            md5, sha256 = get_hash_values(img_path)
            data.append({
                "圖檔": img_path,
                "檔名": img_path.name,
                "說明": desc,
    ...
    if data:
        ...
        with io.BytesIO() as output:
            ...
            header = ["縮圖", "檔名", "說明", "原圖路徑", "修改時間", "拍攝時間", "EXIF狀態", "檔案大小 (KB)", "MD5", "SHA256"]
            ...
            workbook.close()
            st.download_button("📥 下載含縮圖與雜湊 Excel 索引", data=output.getvalue(),
                               file_name="相片索引_with_hash.xlsx",
