...
if uploaded_zip:
    ...
    if data:
        ...
        st.markdown("### 📋 圖片索引表")
        st.dataframe(df.drop(columns=["圖檔"]))  # 不顯示圖檔物件欄

        # 匯出含圖片 Excel
        with io.BytesIO() as output:
            workbook = xlsxwriter.Workbook(output, {'in_memory': True})
            worksheet = workbook.add_worksheet("相片索引表")
            header = ["縮圖", "檔名", "說明", "原圖路徑", "修改時間", "建立時間", "檔案大小 (KB)"]
            ...
            for row_num, row in enumerate(data, start=1):
                ...
                worksheet.insert_image(row_num, 0, row["檔名"], {'image_data': img_bytes, 'x_scale': 1, 'y_scale': 1})
                # 寫入其他欄位
                ...
                worksheet.write(row_num, 5, row["建立時間"])
                worksheet.write(row_num, 6, row["檔案大小 (KB)"])

            workbook.close()
            st.download_button("📥 下載含縮圖 Excel 索引", data=output.getvalue(),
                               file_name="相片索引_with_img.xlsx",
